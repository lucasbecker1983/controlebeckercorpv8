  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-full w-[var(--sidebar-width)] flex-col border-r border-outline/12 bg-surface-low/92 shadow-[var(--shadow-medium)] backdrop-blur-[18px] transition-transform duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="border-b border-outline/10 px-[var(--sidebar-padding)] py-[var(--sidebar-padding)]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="inline-flex rounded-full border border-primary/16 bg-primary/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-primary">
                Controle Becker Corp
              </div>
              <h2 className="mt-2.5 text-base font-black tracking-tight text-on-surface xl:text-lg">V8 Core</h2>
              <p className="mt-1 text-xs text-on-surface/54 xl:text-sm">Operação corporativa unificada</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="border-b border-outline/10 px-[var(--sidebar-padding)] py-[var(--sidebar-section-padding)]">
          <div className="rounded-[calc(var(--surface-radius)-2px)] border border-outline/12 bg-surface-high/62 p-[var(--spacing-card)]">
            <div className="text-[10px] font-black uppercase tracking-[0.18em] text-on-surface/40">Sessão atual</div>
            <div className="mt-2 text-sm font-semibold text-on-surface">{user?.username || 'Operador'}</div>
            <div className="mt-1 text-xs text-on-surface/54 xl:text-sm">Painel institucional premium em dark mode MD3.</div>
          </div>
        </div>
        ...
        <div className="border-t border-outline/10 p-[var(--sidebar-section-padding)]">
          <button
            onClick={onLogout}
            className="flex min-h-[var(--control-height)] w-full items-center justify-center gap-2 rounded-[calc(var(--surface-radius)-4px)] border border-danger/18 bg-danger/10 px-[var(--control-padding-x)] py-2 text-[11px] font-black uppercase tracking-[0.16em] text-danger transition-all hover:bg-danger hover:text-white"
          >
            <LogOut size={16} />
            Sair do sistema
          </button>
