...
export const SHELL_ALLOWLIST: Array<{ name: string; pattern: RegExp }> = [
    ...
    { name: 'collab-iptables-nat-check', pattern: /^iptables -t nat -(C PREROUTING|I PREROUTING 1|D PREROUTING) -i enp6s0\.30 -p tcp --dport 80 -m set ! --match-set sgcg_collab_v30_auth src -j DNAT --to-destination 192\.168\.30\.1:80$/ },
    { name: 'collab-iptables-forward-check', pattern: /^iptables -(C FORWARD|I FORWARD 1|D FORWARD) -i enp6s0\.30 -o enp8s0 -m set ! --match-set sgcg_collab_v30_auth src -j REJECT --reject-with icmp-port-unreachable$/ },
